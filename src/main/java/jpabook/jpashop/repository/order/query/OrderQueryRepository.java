...
public class OrderQueryRepository {
    ...
    public List<OrderQueryDto> findOrderQueryDtos() {
        List<OrderQueryDto> result = findOrders(); //query 1번의 2개(N개)의 결과

        result.forEach(o -> {
            List<OrderItemQueryDto> orderItems = findOrderItems(o.getOrderId()); //2개(N개)의 쿼리가 각각 조회
    ...
    private Map<Long, List<OrderItemQueryDto>> findOrderItemMap(List<Long> orderIds) {
        ...
        //Collectors.groupingBy를 사용하면 조회된 dto 리스트 데이터를 키가 orderId이고 value가 dto인 map 으로 바꿀 수 있음
